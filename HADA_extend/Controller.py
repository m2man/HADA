...
class Controller(nn.Module):
    def __init__(self, config):
        ...
        self.weight_albef = nn.Parameter(torch.ones([]) * 0.5)  
        # self.weight_md = nn.Parameter(torch.ones([]) * 1)  
        
        # MODEL
        self.img_encoder = MH(ft_trans=self.ft_trans, ft_gcn=self.ft_gcn, ft_com=self.ft_com,
        ...
        self.cap_encoder = MH(ft_trans=self.ft_trans, ft_gcn=self.ft_gcn, ft_com=self.ft_com,
                                 n_heads=self.n_heads, type_gcn=self.type_gcn, skip=self.skip,
                                 batch_norm=self.batch_norm, dropout=self.dropout, act_func=self.act_func)
        ...
        self.img_encoder_m = MH(ft_trans=self.ft_trans, ft_gcn=self.ft_gcn, ft_com=self.ft_com,
                                 n_heads=self.n_heads, type_gcn=self.type_gcn, skip=self.skip,
                                 batch_norm=self.batch_norm, dropout=self.dropout, act_func=self.act_func)
        self.cap_encoder_m = MH(ft_trans=self.ft_trans, ft_gcn=self.ft_gcn, ft_com=self.ft_com,
                                 n_heads=self.n_heads, type_gcn=self.type_gcn, skip=self.skip,
                                 batch_norm=self.batch_norm, dropout=self.dropout, act_func=self.act_func)
        ...
        self.params += [self.weight_albef] 
        # self.params += [self.weight_md]
        ...
        self.register_buffer("image_queue_a", torch.randn(256, self.queue_size))
        self.register_buffer("text_queue_a", torch.randn(256, self.queue_size))
        ...
        self.image_queue_a = F.normalize(self.image_queue_a, dim=0).to(self.device)
        self.text_queue_a = F.normalize(self.text_queue_a, dim=0).to(self.device)
    ...
    def load_model(self, save_path=None, weight_only=False):
        ...
        if save_path is not None:
            ...
            if not weight_only:
                ...
                self.weight_albef = modelCheckpoint['weight_albef']
                # self.weight_md = modelCheckpoint['weight_md']
    ...
    def save_model(self, loss, epochID=0, save_path='', optimizer=True):
        if optimizer:
            torch.save({'epoch': epochID,
                        ...
                        'weight_albef': self.weight_albef,
                        # 'weight_md': self.weight_md,
    ...
    def forward_batch(self, batch):
        with torch.no_grad():
            ...
            # self.weight_albef.clamp_(0.001, 0.9)
        ...
        img_enc_a = img_dict['cls_albef_ori']
        cap_enc_a = cap_dict['cls_albef_ori']
        ...
        if self.l2_norm:
            ...
            # Extend Original ALBEF
            img_enc_a = F.normalize(img_enc_a, dim=1)
            cap_enc_a = F.normalize(cap_enc_a, dim=1)
        ...
        with torch.no_grad():
            ...
            img_enc_a_all = torch.cat([img_enc_a.t(),self.image_queue_a.clone().detach()],dim=1) 
            cap_enc_a_all = torch.cat([cap_enc_a.t(),self.text_queue_a.clone().detach()],dim=1)

            if self.distill:               
                sim_i2t_m = img_enc_m @ cap_enc_all / self.temp_para
                sim_t2i_m = cap_enc_m @ img_enc_all / self.temp_para   
                # sim_i2t_a_m = img_enc_a @ cap_enc_a_all / self.temp_para
                # sim_t2i_a_m = cap_enc_a @ img_enc_a_all / self.temp_para
                # sim_i2t_combine_m = (1-self.weight_albef)*sim_i2t_m + self.weight_albef*sim_i2t_a_m
                # sim_t2i_combine_m = (1-self.weight_albef)*sim_t2i_m + self.weight_albef*sim_t2i_a_m
        ...
        sim_i2t_a = img_enc_a @ cap_enc_a_all / self.temp_para
        sim_t2i_a = cap_enc_a @ img_enc_a_all / self.temp_para 
        
        sim_i2t_combine = (1-self.weight_albef)*sim_i2t + self.weight_albef*sim_i2t_a
        sim_t2i_combine = (1-self.weight_albef)*sim_t2i + self.weight_albef*sim_t2i_a
        ...
        self._dequeue_and_enqueue(img_enc_m, cap_enc_m, img_enc_a, cap_enc_a, idx)
    ...
    def train_epoch(self, dataloader, epochID=0, writer=None):
        ...
        for idx, batch in enumerate(dataloader):
            ...
            with torch.no_grad():
                # self.weight_albef.clamp_(0.1, min(0.9, 0.7 + epochID*0.05))
                self.weight_albef.clamp_(min=0.1, max=0.9)
            ...
            if writer is not None:
                ...
                if (idx+1) % wit == 0 or idx == 0:
                    writer.add_scalars('LearnPara', {'wa': self.weight_albef.item()}, epochID * np.floor(numb_iter/wit) + np.floor((idx+1)/wit))
                    # writer.add_scalars('LearnPara', {'wm': self.weight_md.item()}, epochID * np.floor(numb_iter/wit) + np.floor((idx+1)/wit))
                    if self.temp > 0:
    ...
    def evaluate_with_list_id(self, list_img_id, list_cap_id, para_standardize=None, apply_temp=True, return_sim=False):
        ...
        img_enc, img_enc_a = self.eval_encode(dataloader_img, branch='img')
        cap_enc, cap_enc_a = self.eval_encode(dataloader_txt, branch='txt')
        with torch.no_grad():
            if apply_temp:
                sims = img_enc @ cap_enc.T / self.temp_para
                sims_a = img_enc_a @ cap_enc_a.T / self.temp_para
            else:
                sims = img_enc @ cap_enc.T 
                sims_a = img_enc_a @ cap_enc_a.T 
            sims = (1-self.weight_albef)*sims + self.weight_albef * sims_a
            sims = sims.cpu().numpy()
            ...
            for idx_epoch in range(num_epoch):
                ...
                if loss_update <= (loss_best-self.thres_loss):
                    ...
                    metrics = {'temp_para': self.temp_para.item(),
                               'weight_albef': self.weight_albef.item(),
                               ...
                               'Cweight_albef': self.weight_albef.item(),
                ...
                info_txt += f"TR1i: {np.round(tr1i,6)}\nTR5i: {np.round(tr5i,6)}\nTR10i: {np.round(tr10i,6)}\n"
                info_txt += f"TR1t: {np.round(tr1t,6)}\nTR5t: {np.round(tr5t,6)}\nTR10t: {np.round(tr10t,6)}\n"
                info_txt += f"TRi: {np.round(tr1i+tr5i+tr10i,6)}\nTRt: {np.round(tr1t+tr5t+tr10t,6)}\n"
                info_txt += f"TRall: {np.round(tr1i+tr5i+tr10i+tr1t+tr5t+tr10t,6)}\n"
    ...
    def eval_encode(self, dataloader, branch='img'):
        ...
        with torch.no_grad():
            for idx, batch in enumerate(dataloader):
                ...
                x_enc_a = x_dict['cls_albef_ori']
                # Normalize ???
                if self.l2_norm:
                    x_enc = F.normalize(x_enc, dim=1)
                    x_enc_a = F.normalize(x_enc_a, dim=1)
                list_enc = torch.cat((list_enc, x_enc.detach().cpu()), 0)
                list_enc_a = torch.cat((list_enc_a, x_enc_a.detach().cpu()), 0)
        return list_enc, list_enc_a
    ...
    @torch.no_grad()
    def _dequeue_and_enqueue(self, image_feat, text_feat, image_feat_2, text_feat_2, idx):
        ...
        self.image_queue_a[:, ptr:ptr + batch_size] = image_feat_2.T
        self.text_queue_a[:, ptr:ptr + batch_size] = text_feat_2.T
